...
5. **Agent 模式（Coding Agent）** — 除了 Team 模式（单次 LLM 模拟多角色）外，新增了真正的 **Agent 模式**：
...
| `E2B_API_KEY` | 否 | E2B 沙箱 API Key（Agent 模式真实命令执行） |
| `NEON_API_KEY` | 否 | Neon API Key（Agent 模式自动创建数据库） |
...
- [x] 工程师/团队/Agent 三种模式
- [x] Agent 模式 — ReAct 循环引擎 + Tool Use（read_file/write_file/run_command）
- [x] Agent Multi-Agent 编排（Emma→Bob→Alex 流水线）
- [x] E2B 沙箱支持（可选，真实命令执行）
- [x] Neon 数据库自动创建（可选）
